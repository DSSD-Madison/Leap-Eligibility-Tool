    ...
    <div ref={containerRef} className="relative cursor-pointer w-full px-4">
      {tooltip.show && (
        ...
        >
          ...
          <div className="absolute left-1/2 bottom-0 transform -translate-x-1/2 translate-y-1/2 rotate-45 w-2 h-2 bg-white" />
        </div>
      )}
      <ComposableMap
          ...
          {({ geographies }) =>
            geographies.map((geo) => (
              <Geography
                key={geo.rsmKey}
                onMouseMove={(event) =>
                  handleMouseMove(event, geo.properties.NAME)
                }

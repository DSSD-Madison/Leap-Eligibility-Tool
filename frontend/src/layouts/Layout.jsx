...
const Layout = ({ children }) => {
  ...
  const getCountyInfo = (topology) => {
    return topology.objects.tl_2024_us_county.geometries
      .map(geometry => ({
  ...
  const isCountyValid = (countyID) => {
    try {
      const valid =
        mapData.objects.tl_2024_us_county.geometries
  ...
  const covertCountyIDToName = (countyID) => {
    if(!isCountyValid(countyID)) {
      return '';
    }
    const county = mapData.objects.tl_2024_us_county.geometries
      .find(geometry => geometry.properties.GEOID === countyID);
  ...
  return (
    <SearchContext.Provider value={{ searchQuery, setSearchQuery, isCountyValid, covertCountyIDToName }}>
